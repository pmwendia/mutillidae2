...
RUN apt-get install -y php-dev gcc make autoconf libc-dev pkg-config libmcrypt-dev sudo nano
...
RUN sed -i 's/^Deny from all/Allow from all/g' /var/www/html/.htaccess

# Update mutillidae database config to use the mysql docker instance
RUN sed -i 's/127.0.0.1/db/' /var/www/html/includes/database-config.inc
